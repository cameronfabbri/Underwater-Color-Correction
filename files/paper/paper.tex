...
this embedding is then upsampled (decode) via transpose convolutions to reconstruct the image. The advantage of using
...
by a leaky ReLU activation with slope $0.02$, while transpose convolutions in the encoder are followed by a ReLU
activation \cite{nair2010rectified}. Excempt from this is the last layer of the decoder, which uses a TanH nonlinearity
to match the input distribution of $[-1, 1]$. Following \cite{radford2015unsupervised}, we use batch normalization
\cite{pmlr-v37-ioffe15} throughout the generator, except for the last layer of the decoder. 
...
batch size of 32, Adam Optimizer \cite{kingma2014adam} with learning rate $1e-4$, and train for 100 epochs.
Following WGAN-GP, the discriminator is updated $n$ times for every update of the generator, where $n = 5$. Our
implementation was done using the Tensorflow library \cite{abadi2016tensorflow}.
...
We trained UGAN and UGAN-P on the image pairs generated by CycleGAN, and evaluated on the images from the test set, $Y$.
Note that these images do not contain any ground truth, as they are original distorted images from Imagenet. Images for
training and testing are of size $256 \times 256 \times 3$ and normalized between $[-1, 1]$.
