...
gradient norm of the discriminator's output with respect to its input. Following \cite{gulrajani2017improved}, the new objective then becomes

\begin{equation}
\begin{aligned}
   \mathcal{L}_{WGAN} = \mathbb{E} [D(I^C)] - \mathbb{E} & [D(G(I^D))] + \\
...
\begin{equation}
   \begin{aligned}
      \mathcal{L}_{UGAN} = \mathcal{L}_{WGAN} + \mathcal{L}_{L1}.
   \end{aligned}
\end{equation}
...
\noindent In our experiments, we denote our network as UGAN-D when considering the GDL, which can be expressed as

\begin{equation}
   \begin{aligned}
      \mathcal{L}_{UGAN-D} = \mathcal{L}_{WGAN} + \mathcal{L}_{L1} + \mathcal{L}_{GDL}.
   \end{aligned}
\end{equation}


\subsection{Network Architecture and Training Details}
We use a similar architecture as \cite{isola2016image}. Due to the Our generator is a U-Net
