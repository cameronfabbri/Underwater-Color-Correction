...
%Batch normalization \cite{pmlr-v37-ioffe15} is used in the encoder portion
%of the generator, but not in the decoder.
%Following \cite{radford2015unsupervised}, we use batch normalization
...
acquired from Flickr \footnote{How do I cite Flickr}.

\subsection{Evaluation}
We train UGAN and UGAN-P on the image pairs generated by CycleGAN, and evaluate entirely on the images from the
...
Figure 3 shows a qualitative 
For a qualitative evaluation, we compare against CycleGAN 
...
\end{tabular}
\caption{Samples. The network can both recover color and also correct color if a small amount is present.}
\end{figure}
